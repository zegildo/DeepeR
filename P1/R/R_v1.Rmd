...
dataset_builder <- function(qt_dataset){
  ...
  dados <- tibble(
    LETRAS = paste0(
      sample(LETTERS, qt_dataset, replace = TRUE), 
      sample(seq_len(qt_dataset ^ (1/3)), qt_dataset, replace = TRUE)
    ),
    NUMEROS = sample(seq_len(qt_dataset), qt_dataset, replace = TRUE)
  )
...
tomas <- function(tabela){
  ...
  map_df(tabela, ~map_int(tabela, qtd_in, b = .x)) %>%
            mutate(linha = names(tabela)) %>%
            pivot_longer(-linha) %>% 
    set_names(c('letra_i', 'letra_j', 'peso'))
...
bech_mark <- function(size){
  ...
  comp <- microbenchmark(#"fernando" = fernando(tabela),
                       "tomas" = tomas(tabela), 
                       "joao" = joao(tabela), 
                       times = 10, 
                       unit = "ms") 
...
# Melhor para aqui porque o bench já está muito lento.
